...
describe("normal init with default settings", () => {
  ...
  // TODO: Add test for cases where `inquirer` prompts are used for this.
    test("basic inquirer test to check if its working", async () => {
      ...
      Object.values(templates).forEach((template, index) => {
        ...
        expect(getScreen()).toMatchInlineSnapshot(`"√ Select a template ${template.name}"`);
      });

      const { answer: answer1, events: events1, getScreen: getScreen1 } = await render(input, {
        message: "Enter a name for your server app (default: qse-server)"
      });
      expect(getScreen1()).toMatchInlineSnapshot(`"? Enter a name for your server app (default: qse-server)"`);
      events1.type('qse');
      events1.keypress('enter');
      await expect(answer1).resolves.toEqual('qse');

      // const { answer: answer2, events: events2, getScreen: getScreen2 } = await render(confirm, {
      //   message: "Do you want nodemon hot-reload support? (default: Yes)"
      // });

      // events2.keypress('n');
      // events2.keypress('enter');
      // await expect(answer2).resolves.toEqual(false);

      // events2.keypress('N');
      // events2.keypress('enter');
      // expect(answer2).resolves.toEqual(false);

      // events2.keypress('y');
      // events2.keypress('enter');
      // await expect(answer2).resolves.toEqual(true);
